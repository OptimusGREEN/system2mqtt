...
class prox_auth:
    ...
    def setup_connection(self):
        ...
        self.returned_data = {
            'status': {'code': self.response.status_code, 'ok': self.response.ok, 'reason': self.response.reason}}
...
class OptiMOX(object):
    ...
    def connect(self, conn_type, option, post_data):
        ...
        try:
            ...
            not sys._getframe(1).f_code.co_name == sys._getframe(0).f_code.co_name):
                print
                "Unexpected error: %s : %s" % (str(sys.exc_info()[0]), str(sys.exc_info()[1]))
                print
                "try to recover connection auth"
    ...
    def get(self, api_url_tail):
        """Get anything you like by api url"""
        data = self.connect('get', api_url_tail, None)
        return data
    ...
    def getNodeReport(self, node):
        """Read node report. Returns JSON"""
        data = self.connect('get', 'nodes/%s/report' % (node), None)
        return data
